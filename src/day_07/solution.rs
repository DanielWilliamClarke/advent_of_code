...
impl Solution<String, u32> for Day07 {
    fn pt_1(&self, input: &[String]) -> u32 {
        self.optimize(&self.parse(input), |n| n)
    }

    fn pt_2(&self, input: &[String]) -> u32 {
        self.optimize(&self.parse(input), |n| n * (n + 1) / 2)
    }
}
    ...
    fn optimize(&self, input: &[u32], cost_predicate: fn(u32) -> u32) -> u32 {
        ...
        (*min..*max)
            .map(|index| {
                input
                    .iter()
                    .map(|position| cost_predicate(index.abs_diff(*position)))
                    .sum::<u32>()
            })
            .min()
            .unwrap()
...
mod tests {
    ...
    fn solution_is_correct() {
        ...
        vec![(day.pt_1(&input), 347449), (day.pt_2(&input), 98039527)]
            .iter()
